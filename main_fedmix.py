...
from models.Update import ClientUpdate_fedmix, ServerUpdate_fedavg, batch_generator
from models.Nets import QuickModel, CNNCifar, CNNMnist, LSTM
from models.Fed import FedAvg, FedFreq
...
if __name__ == '__main__':
    ...
    for _ in range(args.repeat):
        ...
        trans_cifar_aug = transforms.Compose([
        ...
        dataset_train_aug = datasets.CIFAR10('../dataset/cifar', train=True, download=True, transform=trans_cifar_aug)
        ...
        for iter in range(args.epochs):
            ...
            model_local_static_0 = copy.deepcopy(net_glob).to(args.device)
            ...
            model_local_static = copy.deepcopy(net_glob).to(args.device)
            

            for idx in idxs_users:
                local = ClientUpdate_fedmix(
                    args, 
                    dataset_train,
                    dataset_train_aug,
                    dict_users_unlabeled[idx]
                    )
                w, loss = local.train(copy.deepcopy(net_glob).to(args.device), model_local_static)
            ...
            w_glob = FedFreq(w_glob, model_local_static.state_dict(), model_local_static_0.state_dict())
    ...
    # plt.show()
    fig.savefig('fedmix' + '_' + str(args.init_epochs) + '_' + str(args.threshold) + '_' + args.iid + '.png')
